...
$(".estatisticas_load").html(`<svg aria-hidden="true" class="inline w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    axios.get('usuarios/estatisticas')
        .then(response => {
            ...
            for (let i in lista_projetos) {
                let projeto = `<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">                        
                                <td class="px-6 py-4">
...
function badge_projeto(status) {
    let badge = "";
    switch (status) {
        case "Completo": badge = `<span class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">Completo</span>`; break;
        case "Em andamento": badge = `<span class="bg-yellow-100 text-yellow-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-yellow-900 dark:text-yellow-300">Em Andamento</span>`; break;
        default: badge = `<span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-blue-900 dark:text-blue-300">${status}</span>`; break;
    }
    return badge;
}
