import{g as i,e as o,b as n}from"./funcoes-CNXYSAtO.js";function p(){$("#formularios").empty(),axios("formularios/lista").then(s=>{let t=s.data;console.log(t);for(let a in t){let e=`  <a href="/formularios/formulario/${t[a].id}" class="mb-4 w-full block p-6 bg-white border border-black-800 rounded-lg shadow-sm hover:bg-gray-100">
                                    ...
                                    </a>`;$("#formularios").append(e)}}).catch(s=>{o(s)}).finally(()=>{})}function u(){let s=$("#formulario_id").val();$("#perguntas_respondidas").empty(),axios.get(app_url+"/formularios/respostas/formulario/"+s).then(t=>{$("#qtd_perguntas_respondidas").html(`(${t.data.length})`);let a=t.data;for(let e in a){let r=a[e].arquivo_id==null?"<p>(SEM FOTO)</p>":`<img src="${app_url+"/arquivos/exibir/"+a[e].arquivo_id}">`,l=`<tr class="bg-white">                        
                            <td class="px-6 py-4">
                                <p>Pergunta: ${a[e].pergunta.titulo}</p>
                                <p>Resposta: ${a[e].resposta}</p>
                                <p>Responsável: ${a[e].usuario.nome}</p>
                                <p>Momento do cadastro: ${n(a[e].data_cadastro,!0)}</p>                                
                                ${r}                                                                
